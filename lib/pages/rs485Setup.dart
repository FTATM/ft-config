...
/// 3. โหมดส่งออกข้อมูล เลือกได้ 4 แบบ: HTTP API / MQTT / TCP / UDP (แต่ละแบบมี config แยกกัน)
...
class _Rs485SetupPageState extends State<Rs485SetupPage> {
  ...
  // ---------- Output mode ----------
  String _outputMode = 'http'; // 'http' | 'mqtt' | 'tcp' | 'udp'
  ...
  bool _mqttUseTls = false; // true = ต่อแบบ TLS ด้วย cert ที่ฝังไว้ในเฟิร์มแวร์ (เช่น AWS IoT Core) แทน username/password
  ...
  bool _loadingTcpOutput = true;
  bool _savingTcpOutput = false;
  String? _tcpOutputError;
  ...
  // =====================================================
  // GET/POST /api/rs485/output_tcp
  // =====================================================
  Future<void> _loadTcpOutput() async {
    setState(() {
      _loadingTcpOutput = true;
      _tcpOutputError = null;
    });

    try {
      final res = await http.get(Uri.parse('${widget.baseUrl}/api/rs485/output_tcp')).timeout(_timeout);
      if (!mounted) return;

      if (res.statusCode == 200) {
        final data = jsonDecode(res.body) as Map<String, dynamic>;
        _tcpHostController.text = data['host']?.toString() ?? '';
        _tcpPortController.text = '${data['port'] ?? ''}';
        setState(() => _loadingTcpOutput = false);
      } else {
        setState(() {
          _tcpOutputError = 'โหลดค่าไม่สำเร็จ (${res.statusCode})';
          _loadingTcpOutput = false;
        });
      }
    } on TimeoutException {
      if (!mounted) return;
      setState(() {
        _tcpOutputError = 'หมดเวลาเชื่อมต่อ';
        _loadingTcpOutput = false;
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _tcpOutputError = 'เกิดข้อผิดพลาด: $e';
        _loadingTcpOutput = false;
      });
    }
  }

  Future<void> _saveTcpOutput() async {
    if (_tcpHostController.text.trim().isEmpty) {
      _showMessage('กรุณากรอก Host ปลายทาง');
      return;
    }

    setState(() => _savingTcpOutput = true);

    final body = {
      "host": _tcpHostController.text.trim(),
      "port": int.tryParse(_tcpPortController.text.trim()) ?? 0,
    };

    try {
      final res = await http
          .post(
            Uri.parse('${widget.baseUrl}/api/rs485/output_tcp'),
            headers: {'Content-Type': 'application/json'},
            body: jsonEncode(body),
          )
          .timeout(_timeout);
      ...
      final data = jsonDecode(res.body) as Map<String, dynamic>?;
      final success = res.statusCode == 200 && data?['success'] == true;
      _showMessage(success ? 'บันทึกการตั้งค่า TCP สำเร็จ' : (data?['message']?.toString() ?? 'บันทึกไม่สำเร็จ'));
    } on TimeoutException {
      if (!mounted) return;
      _showMessage('หมดเวลาเชื่อมต่อ');
    } catch (e) {
      ...
      if (mounted) setState(() => _savingTcpOutput = false);
    }
  }
              ...
              Column(
                children: [
                  ...
                  RadioListTile<String>(
                    ...
                    subtitle: const Text("เปิด connection แล้วส่ง JSON ไปยัง TCP server ปลายทาง การันตีถึงปลายทางมากกว่า UDP"),
                    value: 'tcp',
                    groupValue: _outputMode,
                    onChanged: _savingOutputMode ? null : _changeOutputMode,
                  ),
        ...
        child: Column(
          ...
          children: [
            ...
            if (_loadingMqttOutput)
              ...
              SwitchListTile(
                contentPadding: EdgeInsets.zero,
                title: const Text("เชื่อมต่อแบบ TLS"),
                subtitle: const Text("เปิดเมื่อต่อ broker ที่ต้องใช้ cert เช่น AWS IoT Core (มักใช้ port 8883)"),
                value: _mqttUseTls,
                onChanged: (v) {
                  setState(() {
                    _mqttUseTls = v;
                    // auto-suggest port ตามโหมด แต่ไม่ทับ port ที่ผู้ใช้เคยแก้เองมาก่อน
                    final currentPort = _mqttPortController.text.trim();
                    if (v && (currentPort.isEmpty || currentPort == '1883')) {
                      _mqttPortController.text = '8883';
                    } else if (!v && currentPort == '8883') {
              ...
              const SizedBox(height: 4),
              if (_mqttUseTls) ...[
                // TLS ยืนยันตัวตนด้วย cert ที่ฝังไว้ในเฟิร์มแวร์ ไม่ใช้ username/password
                Container(
                  ...
                  child: const Text(
                    "โหมด TLS ใช้ certificate ที่ฝังไว้ในเฟิร์มแวร์ยืนยันตัวตน ไม่ต้องกรอก Username/Password",
  ...
  // ---------------------------------------------------
  // Card: TCP output config
  // ---------------------------------------------------
  Widget _buildTcpOutputCard() {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text("ตั้งค่า TCP", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
            const SizedBox(height: 12),
            if (_loadingTcpOutput)
              const Center(child: CircularProgressIndicator())
            else ...[
              if (_tcpOutputError != null) ...[
                Text(_tcpOutputError!, style: const TextStyle(color: Colors.red)),
                const SizedBox(height: 8),
              ],
              Row(
                children: [
              ...
              SizedBox(
                width: double.infinity,
                child: ElevatedButton.icon(
                  onPressed: _savingTcpOutput ? null : _saveTcpOutput,
                  icon: _savingTcpOutput
                      ? const SizedBox(height: 16, width: 16, child: CircularProgressIndicator(strokeWidth: 2))
                      : const Icon(Icons.save),
                  label: const Text("บันทึกการตั้งค่า TCP"),
                ),
              ),
            ],
          ],
