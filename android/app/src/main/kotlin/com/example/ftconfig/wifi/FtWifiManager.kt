...
class FtWifiManager(
    ...
    // ---------------------------------------------------------------------
    // Multicast lock (จำเป็นสำหรับ mDNS discovery — Android บล็อก multicast
    // packet โดยดีฟอลต์เพื่อประหยัดแบตฯ ต้อง acquire lock นี้ก่อนถึงจะรับ
    // mDNS response กลับมาได้ ไม่งั้น multicast_dns ฝั่ง Dart จะเงียบเฉยๆ)
    // ---------------------------------------------------------------------
    ...
    fun acquireMulticastLock() {
        ...
        val lock = systemWifiManager.createMulticastLock("ftconfig_mdns_lock")
